'''
System to distinguish the action a message is trying to achieve
...
async def respond_to_message(websocket, message):
    ...
    # Goal: respond to different kinds of messages differently
    type = data.get('type')
    if type == 'login':
    ...
    # when any of the clients sens a message of type "chat",
    # relay the message to anyone who is logged in
    elif type == 'chat':
        print('Broadcast chat message', data)
        for socket in logged_in_users:
            await socket.send(json.dumps(data))

    elif type == 'disconnect':
        print('Stop tracking this websocket')
        ...
        print('Read instructions to format your message correctly')
        websocket.send(json.dumps({'Unrecognized message type:', data}))
    '''
    ******************************************************************
