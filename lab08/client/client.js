...
const initializeConnection = ev => {
    ...
    connection.onmessage = e => {
        ...
        if (data.type == "login" || data.type == "disconnect") {
            let users_html = ''
            data.users.forEach(username => {users_html += `<li>${username}</li>`})
            qs("#users-list").innerHTML = `<ul>${users_html}</ul>`
        }
        else if (data.type == "chat") {
            let chat_html = qs("#chat").innerHTML
